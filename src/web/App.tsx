...
export function App() {
  ...
  const saveDecision = async (payload: Record<string, unknown>) => {
    if (!snapshot) return;
    setBusy(true); setError("");
    try { const response = await apiRequest<CreateCaseResponse>(`/api/cases/${snapshot.input.caseId}/revisions/${snapshot.manifest.revisionId}/decision`, { method: "POST", body: JSON.stringify(payload) }); commitSnapshot(response.snapshot); setVerificationOpen(false); setNotice("已选择结果，现在可以复制给 AI。"); await loadCases(snapshot.input.caseId); } catch (reason) { setError(reason instanceof Error ? reason.message : "选择未保存"); } finally { setBusy(false); }
  };
  ...
  const riskNotice = useMemo(() => snapshot && (retainedSnapshotRisk || candidateDecisionRequired) ? retainedSnapshotRisk ? "本次更新未成功，当前显示的是上一次成功结果。" : "有多个可能结果，请先选择一个。" : "", [candidateDecisionRequired, retainedSnapshotRisk, snapshot]);
